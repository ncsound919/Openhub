...
export function WikiView() {
  ...
  const wikiPages = useStore(useShallow(state => state.wikiPages.filter(i => i.repoId === repo?.id)));
        ...
        {homePage ? (
          <div className="border border-border-muted rounded-md bg-surface-raised shadow-sm overflow-hidden">
            <div className="bg-surface-base border-b border-border-muted px-4 py-3 flex items-center justify-between">
               <h1 className="text-lg font-semibold text-gray-400">{homePage.title}</h1>
               <div className="flex space-x-2">
                  <button className="px-3 py-1.5 text-sm font-medium border border-border-muted rounded-md hover:bg-surface-raised bg-surface-raised flex items-center">
            ...
            <div className="bg-surface-base border-t border-border-muted px-4 py-3 text-xs text-gray-400 text-right">
              developer edited this page {formatDistanceToNow(new Date(homePage.updatedAt))} ago
            </div>
          </div>
        ) : (
          <div className="p-12 text-center text-gray-400 border border-border-muted rounded-md">
            <FileText className="w-8 h-8 text-gray-400 mx-auto mb-4" />
            <h3 className="text-lg font-semibold text-gray-400">Welcome to the wiki!</h3>
        ...
        <div className="border border-border-muted rounded-md bg-surface-raised overflow-hidden shadow-sm">
           <div className="px-4 py-3 border-b border-border-muted">
             <div className="relative">
                ...
                <input type="text" placeholder="Find a page..." className="w-full pl-9 pr-3 py-1.5 border rounded-md border-border-muted bg-surface-base text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-surface-raised" />
             </div>
           </div>
           <div className="p-4">
             <h3 className="text-sm font-semibold text-gray-400 mb-3">Pages <span className="bg-surface-raised rounded-full px-2 py-0.5 ml-1 text-xs text-gray-400 ">{wikiPages.length}</span></h3>
