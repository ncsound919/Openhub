  ...
  return (
    <div className="flex-1 max-w-7xl mx-auto w-full flex flex-col gap-8 px-4 py-8 relative z-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-4">
         <div>
            <h1 className="text-[var(--color-text-primary)] font-industrial text-4xl flex items-center">
              <Terminal className="w-8 h-8 mr-4 text-green-500" /> Workspace Intelligence
            </h1>
            <p className="text-gray-400 font-mono text-[10px] uppercase tracking-widest mt-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
         ...
         <div className="industrial-card p-6 flex flex-col h-[500px]">
            <div className="flex items-center justify-between border-b border-border-muted pb-4 mb-4">
               <h2 className="text-[var(--color-text-primary)] font-industrial text-xl flex items-center">
                  <GitBranch className="w-5 h-5 mr-3 text-orange-500" /> Version Control
               </h2>
               <button onClick={fetchGitData} aria-label="Refresh version control data" title="Refresh version control data" className="text-gray-400 hover:text-[var(--color-text-primary)] transition-colors">
         ...
         <div className="flex flex-col gap-8 h-[500px]">
            ...
            <div className="industrial-card p-6 flex-1 flex flex-col">
              <div className="flex items-center justify-between border-b border-border-muted pb-4 mb-4">
                 ...
                 <button onClick={fetchHygiene} aria-label="Refresh repo hygiene" title="Refresh repo hygiene" className="text-gray-400 hover:text-[var(--color-text-primary)] transition-colors">
                   <RefreshCw className="w-4 h-4" />
                 </button>
              </div>
            ...
            <div className="industrial-card p-6 flex-1 flex flex-col">
              <div className="flex items-center justify-between border-b border-border-muted pb-4 mb-4">
                 ...
                 <button onClick={fetchDeps} aria-label="Refresh dependency alignment" title="Refresh dependency alignment" className="text-gray-400 hover:text-[var(--color-text-primary)] transition-colors">
                   <RefreshCw className="w-4 h-4" />
                 </button>
              </div>
                  ...
                  <div className="space-y-2">
                    {Object.entries(outdatedDeps).map(([pkg, info]: [string, any]) => (
                      <div key={pkg} className="flex justify-between items-center text-xs font-mono bg-text-secondary/30 p-2 border border-border-muted">
                        <span className="text-[var(--color-text-primary)]">{pkg}</span>
                        <div className="text-right">
                          <div className="text-gray-400 text-[10px]">Current: {info.current}</div>
        ...
        <div className="flex flex-col gap-4">
          <form onSubmit={handleOllamaSubmit} className="flex gap-4">
            <input 
              ...
              className="flex-1 bg-black border border-border-muted text-[var(--color-text-primary)] px-4 py-2 text-sm font-mono focus:border-purple-500 focus:outline-none transition-colors"
            />
            <button 
              type="submit" 
          ...
          {ollamaResponse && (
            <div className="bg-black/50 border border-border-muted p-4 text-sm font-mono text-gray-400 max-h-[300px] overflow-y-auto whitespace-pre-wrap">
              {ollamaResponse}
            </div>
          )}
        </div>
