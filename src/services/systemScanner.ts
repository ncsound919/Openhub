...
interface TheaterPattern {
  ...
  description: string;
  suggestion: string;
  /** Match against the line with string contents + trailing comments blanked, so
   *  a code pattern is never satisfied by text inside a string/comment. */
  codeOnly?: boolean;
}

/** Blank string-literal contents and trailing line comments (deterministic,
 *  single-line) so `codeOnly` patterns see code, not prose. */
function codeView(line: string): string {
  ...
  for (let i = 0; i < line.length; i++) {
    ...
    if (ch === '/' && line[i + 1] === '/') break; // line comment: drop the rest
    out += ch;
  }
  return out;
}
...
const THEATER_PATTERNS: TheaterPattern[] = [
  // Mock Data & Fixtures
  {
    category: 'mock_data',
    severity: 'high',
    // Require an actual literal value (object/array/string) so a real variable
    // named `sampleSize = 10` or `mockClient = realClient` is NOT flagged.
    regex: /(?:const|let|var)\s+(?:mock|fake|dummy|sample)\w*\s*=\s*[\[{'"`]/i,
  ...
  },
  {
    category: 'stub_implementation',
    severity: 'medium',
    // Keep explicit stub markers; drop "fallback", which is a legitimate
    // offline-degradation pattern and was the top false positive.
    regex: /\/\/\s*(?:stub|unimplemented)\b/i,
  ...
  for (const file of filesToScan) {
    ...
    lines.forEach((lineText, idx) => {
      ...
      for (const pattern of THEATER_PATTERNS) {
        const subject = pattern.codeOnly ? codeView(lineText) : lineText;
...
describe('${baseName} Component', () => {
  it('exports valid component symbols', async () => {
...
describe('${baseName} API Endpoints', () => {
  it('exports a router or handler factory', async () => {
    const mod = await import('${importTarget}');
    expect(mod).toBeDefined();
    ${exports.map(exp => `expect(mod['${exp}']).toBeDefined();`).join('\n    ')}
  });
...
describe('${baseName} Data Store', () => {
  ...
  it('exports its CRUD functions', async () => {
    const mod = await import('${importTarget}');
    expect(mod).toBeDefined();
    ${exports.map(exp => `expect(mod['${exp}']).toBeDefined();`).join('\n    ')}
  });
...
export async function getDeployReadiness(projectDir: string = ROOT_DIR): Promise<DeployReadinessResult> {
  ...
  // Check 1: TypeScript Gate (120s budget: tsc on a real project exceeds 15s)
  let tsPassed = false;
  let tsDetails = '';
  try {
    execSync('npx tsc --noEmit', { cwd: projectDir, stdio: 'pipe', timeout: 120_000 });
    tsPassed = true;
  ...
  // Check 3: Unit Test Suite (120s budget: vitest cold-start exceeds 15s)
  let testsPassed = false;
  let testsDetails = '';
  try {
    const out = execSync('npx vitest run', { cwd: projectDir, stdio: 'pipe', timeout: 120_000 }).toString();
    testsPassed = true;
  ...
  // Check 4: Security & Headers — behavioral where cheap, honest where not.
  // helmet(): verify the middleware is actually registered (app.use(helmet()))
  // rather than merely imported; repo isolation: verify an owner_id predicate
  // exists in a query (not proof of enforcement — details say so explicitly).
  let secPassed = true;
  let secDetails = 'Helmet middleware registered and repository queries scope by owner.';
  const serverPath = path.join(projectDir, 'server.ts');
  if (fs.existsSync(serverPath)) {
    const serverContent = fs.readFileSync(serverPath, 'utf-8');
    const hasHelmetUse = /app\.use\s*\(\s*helmet\s*\(\s*\)/.test(serverContent) || serverContent.includes('helmet(');
    const hasOwnerScope = /owner_id\s*=\s*\?/.test(serverContent);
    if (!hasHelmetUse || !hasOwnerScope) {
      secPassed = false;
      secDetails = `Static heuristic only (not a runtime check): helmet registration ${hasHelmetUse ? 'found' : 'NOT found'}, owner_id scoping ${hasOwnerScope ? 'found' : 'NOT found'}.`;
    } else {
      secDetails = 'Static heuristic: helmet registration + owner_id scoping strings present (runtime behavior not probed).';
    }
  }
  checks.push({
    id: 'security-headers',
